...
void GPIO_Init(void) {
    //Включаем тактирование GPIOA и GPIOC - РАЗЛОЖЕННЫЕ РЕГИСТРЫ
    ...
    *(volatile uint32_t*)(0x40000000U + 0x00020000U + 0x00001000U + 0x00000018U) |= (1 << 2) | (1 << 4);
    ...
    //PA2 (LED1)
    //GPIOA_CRL = PERIPH_BASE + APB2PERIPH_BASE + GPIOA_BASE + CRL_offset (разложение адреса пина на составляющие)
    ...
    *(volatile uint32_t*)(0x40000000U + 0x00010000U + 0x00000800U + 0x00000000U) = 
    ...
    GPIOA_BSRR = GPIOA_BSRR_PA2_RESET;  // Выкл PA2
