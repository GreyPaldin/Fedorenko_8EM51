...
int main(void) {
    ...
    // Начальное состояние - включаем LED2 сразу (убрать, если выставил начальнео состояние 4)
    ...
    while(1) {
        ...
        if (btn1 && !last_btn1 && pa0_mode == 0) {
            // Увеличение шага последовательности зажигания светодиодов (диапозон от 1 до 4)
            leds_sequence++;
            if (leds_sequence > 4) {
                leds_sequence = 1;
            }
